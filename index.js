...
class AutoCreatePage {
  ...
  async existsRootFileType (type) {
    console.log(fs.pathExistsSync(path.join(this.source, type)), '路径存在');

    if (fs.pathExistsSync(path.join(this.source, type))) return type;
    else return null;
  }
  ...
  parseCode () {
    ...
    traverse(ast, {
       ObjectProperty (astPath) {
          ...
          if (key.value === 'pages' && t.isArrayExpression(value)) {
            // 分包
            let root = ''
